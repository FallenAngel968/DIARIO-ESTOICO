...
// 🔥 CONTEXTO QUE FUNCIONA CON FIREBASE WEB

import React, { createContext, useContext, useState, useEffect } from 'react';
import { onAuthStateChanged } from 'firebase/auth';
import { useRouter } from 'expo-router';
import { auth } from '../firebaseConfig.simple';
import { signInWithGoogle, signOutUser } from '../services/auth.service.working';
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (firebaseUser) => {
      console.log('🔄 [AuthProvider] Auth state cambió:', firebaseUser?.email || 'No user');
      ...
      if (firebaseUser) {
        ...
        setUser(userData);
        console.log('✅ [AuthProvider] Usuario establecido:', userData.email);
      } else {
        setUser(null);
        console.log('❌ [AuthProvider] Usuario limpiado');
      }
  ...
  const loginWithGoogle = async () => {
    ...
    try {
      console.log('🔄 [AuthProvider] Login iniciado...');
      const result = await signInWithGoogle();
      
      if (result.success) {
        console.log('✅ [AuthProvider] Login exitoso, navegando...');
        router.push('/(tabs)');
  ...
  const logout = async () => {
    ...
    try {
      ...
      if (result.success) {
        console.log('✅ [AuthProvider] Logout exitoso, navegando...');
